<template>
  <section id="experience-section" class="mb-16">
    <h3
      class="text-2xl font-bold text-neutral-800 dark:text-neutral-100 mb-8 text-center uppercase tracking-wide"
    >
      {{ $t('expTitle') }}
    </h3>
    ...
    <div class="relative">
      <!-- Timeline Line -->
      <div
        class="hidden md:block absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-neutral-200 dark:bg-neutral-700"
      ></div>

      <div
        v-for="(job, index) in visibleJobs"
        :key="job.id"
        class="relative flex flex-col md:flex-row items-center justify-between mb-12 group transition-opacity duration-500"
        :class="[selectedSkill && !matchesSkill(job.desc) ? 'opacity-30 grayscale' : 'opacity-100']"
      >
        <!-- Left Side -->
    ...
    <div v-if="!showAll && allJobs.length > 2" class="mt-8 text-center">
      <button
        class="px-8 py-3 bg-white dark:bg-neutral-800 border-2 border-orange-500 text-orange-600 dark:text-orange-500 font-bold rounded-full hover:bg-orange-500 hover:text-white dark:hover:text-white transition-all shadow-md cursor-pointer uppercase text-sm tracking-wide"
        @click="showAll = true"
      >
        {{ $t('loadMore') }}
      </button>
...
const matchesSkill = (desc) => {
  ...
  const skill = selectedSkill.value.toLowerCase().replace(/ \/ .*/, '')
  return desc.toLowerCase().includes(skill)
}

const allJobs = computed(() => {
  const experiences = tm('experiences')
  // tm can return an object in some environments, ensure it's an array
  return Array.isArray(experiences) ? experiences : Object.values(experiences)
})
const visibleJobs = computed(() => (showAll.value ? allJobs.value : allJobs.value.slice(0, 2)))
</script>
