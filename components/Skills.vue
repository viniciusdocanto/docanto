<template>
  <section
    class="bg-neutral-900 dark:bg-neutral-800/50 rounded-2xl p-8 md:p-12 text-center text-white shadow-2xl border border-transparent dark:border-neutral-700 transition-colors duration-300"
  >
    <h3 class="text-2xl md:text-3xl font-bold mb-2">{{ $t('skillsTitle') }}</h3>
    <p class="text-neutral-400 mb-8">{{ $t('skillsSubtitle') }}</p>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left max-w-4xl mx-auto">
      <!-- Core Skills -->
      <div>
        <h4
          class="text-orange-400 font-semibold mb-4 uppercase text-sm tracking-wider border-b border-neutral-700 dark:border-neutral-600 pb-2"
        >
          Core &amp; Front-end
        </h4>
...
const backendSkills = ['SQL', 'Git / GitHub', 'Unix', 'Docker', 'PHP', 'Wordpress', 'Python']
